...
from app.models.fixes import FixOutcome, FixProposal, PolicyDecision
...
async def collect_context(state: dict[str, Any]) -> dict[str, Any]:
    ...
    ctx = await collect_failure_context(
        event=event,
        ...
        condor_classads=svc.get("condor_classads"),
    )
...
async def apply_fix(state: dict[str, Any]) -> dict[str, Any]:
    """
    Write the versioned execution overlay and any script patches.
    Computes and stores config hashes before and after.
    """
    svc = _svc()
    proposal = _proposal(state)
    ...
    # Apply script patches if the proposal includes them and the controller supports it
